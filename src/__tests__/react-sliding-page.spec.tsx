...
const App = () => {
  const [state, setState] = useState<{isRightPaneOpen?:boolean, isLeftPaneOpen?: boolean}>({
  ...
  return (
    <div>
      <button onClick={() => setState({ isRightPaneOpen: true })} data-testid="open-right-pane">
        Click me to open right pane!
      </button>
      <button onClick={() => setState({ isLeftPaneOpen: true })} data-testid="open-left-pane">
...
describe('render', () => {
  ...
  it('render content on opened state', async () => {
    ...
    expect(screen.getByTestId(RIGHT_PANE_CONTENT)).toHaveTextContent('And I am pane content. BTW, what rocks?');
  });

  it.todo('do not content on toggle state');
});
